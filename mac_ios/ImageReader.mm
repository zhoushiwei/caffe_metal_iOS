...
//    if (file_name){
        ...
        fclose(file_handle);
        file_data_ref = CFDataCreateWithBytesNoCopy(NULL, file_data.data(),
                                                              bytes_in_file,
                                                              kCFAllocatorNull);
...
//    }
    ...
    // try to resize to input layer size
    image = imageWithImage(image, CGSizeMake(input_layer->width(), input_layer->height()));
...
CGImageRef imageWithImage(CGImageRef image, CGSize size) {
