...
#define ENCODE_METAL_MATH_FUNC(classname, number_of_int, number_of_float, count) \
do { \
id<MTLDevice>       device   = ((__bridge id<MTLDevice>)(Caffe::Get().metal_device)); \
id<MTLLibrary>      library  = [device  newDefaultLibrary]; \
id<MTLFunction>     function = [library newFunctionWithName:@#classname]; \
id<MTLBuffer> data_input = ((__bridge id<MTLBuffer>)((void *)in_data)); \
id<MTLBuffer> data_output = ((__bridge id<MTLBuffer>)((void *)out_data)); \
...
id<MTLBuffer> int__ = [device newBufferWithLength:number_of_int*4 options:MTLStorageModeShared]; \
int* p_tmp_int = (int *)[int__ contents]; \
for(int i = 0; i < number_of_int; ++i) { \
p_tmp_int[i] = param_int[i]; \
} \
id<MTLBuffer> float__ = [device newBufferWithLength:number_of_float*4 options:MTLStorageModeShared]; \
float* p_tmp_float = (float *)[float__ contents]; \
for(int i = 0; i < number_of_float; ++i) { \
p_tmp_float[i] = param_float[i]; \
} \
[encoder setBuffer:int__       offset:0 atIndex:0]; \
[encoder setBuffer:float__     offset:0 atIndex:1]; \
[encoder setBuffer:data_input  offset:0 atIndex:2]; \
[encoder setBuffer:data_output offset:0 atIndex:3]; \
[encoder dispatchThreadgroups:numgroups threadsPerThreadgroup:groupsize]; \
[encoder endEncoding]; \
[buffer commit]; \
[buffer waitUntilCompleted]; } while(0)
...
void metal_wrapper(std::string funcname_, void* in_data, const int in_data_size, void* out_data,  const int out_data_size, std::vector<int> para_int, std::vector<float> para_float, int count) {
    ...
    id<MTLBuffer> data_input  = [device newBufferWithBytesNoCopy:in_data  length:((in_data_size * 4)+4095)/4096 * 4096  options:MTLStorageModeShared deallocator:nil];
    id<MTLBuffer> data_output = [device newBufferWithBytesNoCopy:out_data length:((out_data_size * 4)+4095)/4096 * 4096  options:MTLStorageModeShared deallocator:nil];
    ...
    if(para_float.size()) {
        ...
        int* p_tmp_float = (int *)[float__ contents];
...
void caffe_gpu_gemm_metal<float>(
  ...
  int ndb = (metal_TransB) ? K : N;
    ...
    @autoreleasepool{
        ...
        id<MTLDevice> device = MTLCreateSystemDefaultDevice();//((__bridge id<MTLDevice>)(Caffe::Get().metal_device));
        ...
        id<MTLBuffer> a_  = [device newBufferWithBytesNoCopy:(void *)A  length:ceil(M*K/1024.0)*4096  options:MTLStorageModeShared deallocator:^(void *pointer, NSUInteger length){std::cout << pointer << "and the length is " << length << std::endl;}];
        id<MTLBuffer> b_  = [device newBufferWithBytesNoCopy:(void *)B  length:ceil(K*N/1024.0)*4096  options:MTLStorageModeShared deallocator:^(void *pointer, NSUInteger length){std::cout << pointer << "and the length is " << length << std::endl;}];
        id<MTLBuffer> c_  = [device newBufferWithBytesNoCopy:(void *)C  length:ceil(M*N/1024.0)*4096  options:MTLStorageModeShared deallocator:^(void *pointer, NSUInteger length){std::cout << pointer << "and the length is " << length << std::endl;}];
...
//
//
//    MPSMatrix *a = [[MPSMatrix alloc] initWithBuffer:((__bridge id<MTLBuffer>)((void *)A)) descriptor:descA];
//
//    MPSVector *input =  [[MPSVector alloc] initWithBuffer:((__bridge id<MTLBuffer>)((void *)x)) descriptor:descx];
//
//    MPSVector *output = [[MPSVector alloc] initWithBuffer:((__bridge id<MTLBuffer>)((void *)y)) descriptor:descy];
//
//    //a = [a initWithBuffer: ((__bridge id<MTLBuffer>)((void *)A)) options:nil];
//    //MPSMatrix *b;
//    //MPSMatrix *c;
